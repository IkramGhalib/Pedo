        ...
        <div class="form-group col-md-6">
            ...
            <label class="cabinet center-block">
                <figure class="course-image-container">
                    <i data-toggle="tooltip" data-original-title="Delete" data-id="course_image" class="fa fa-trash remove-lp" 
                    data-content="{{  Crypt::encryptString(json_encode(array('model'=>'courses', 'field'=>'course_image', 'pid' => 'id', 'id' => $course->id, 'photo'=>$course->course_image))) }}" 
                    style="display: @if($course->course_image && Storage::exists($course->course_image)){{ 'block' }} @else {{ 'none' }} @endif"></i>
                    <img src="@if($course->course_image && Storage::exists($course->course_image))
                                {{ Storage::url($course->course_image) }}
                                @else{{ asset('backend/assets/images/course_detail.jpg') }}
