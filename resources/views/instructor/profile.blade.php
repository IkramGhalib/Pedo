    		...
    		<div class="form-group col-md-4">
	            ...
	            <label class="cabinet center-block">
	                <figure class="course-image-container">
	                    <i data-toggle="tooltip" data-original-title="Delete" data-id="course_image" class="fa fa-trash remove-lp" data-content="{{  Crypt::encryptString(json_encode(array('model'=>'courses', 'field'=>'course_image', 'pid' => 'id', 'id' => $instructor->id, 'photo'=>$instructor->instructor_image))) }}" style="display: @if($instructor->instructor_image)@if(Storage::exists($instructor->instructor_image)){{ 'block' }}@endif @else {{ 'none' }} @endif "></i>
	                    <img src="@if($instructor->instructor_image)@if(Storage::exists($instructor->instructor_image)){{ Storage::url($instructor->instructor_image) }}@else{{ asset('backend/assets/images/user.jpg') }}@endif @endif" class="gambar img-responsive" id="course_image-output" name="course_image-output" />
...
<script type="text/javascript">
    ...
    { 
        //image crop start
        $(".gambar").attr("src", @if($instructor->instructor_image && Storage::exists($instructor->instructor_image))"{{ Storage::url($instructor->instructor_image) }}" @else "{{ asset('backend/assets/images/course_detail.jpg') }}" @endif);
