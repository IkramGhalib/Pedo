...
class CourseController extends Controller
{
    ...
    public function myCourses(Request $request)
    {
        ...
        $courses = DB::table('courses')
                    ...
                    // $offer_courses = InvoiceDetail::groupBy('invoice_id')->get();
        // dd($courses);
    ...
    public function courseList(Request $request)
    {
        ...
        foreach ($groups as $key => $value) {
            ...
            // $courses = $query->groupBy('courses.id')->get();
    ...
    public function getCourseListAgainstGroup(Request $request)
    {
        ...
        // foreach ($groups as $key => $value) {
            ...
            // ->join('categories', 'categories.id', '=', 'courses.category_id')
            // ->where('courses.is_active',1);
            ->where('categories.group_id',$request->group_id) ->get();
            return response()->json(['result'=>'true','courses'=>$courses]);
    ...
    {   
        $instructor = Instructor::where('status', 'active')->get();
        ...
        return view('instructor.course.create_info', compact('course', 'instruction_levels','instructor'));
    }
    ...
    public function instructorCourseVideo(Request $request, $course_id = '')
    {   
        $course = Course::find($course_id);
        $videos = CourseVideos::where('course_id',$course_id)->get();
        // $video = null;
        // if($course->course_video)
        // {
        //     $video = $this->model->getvideoinfoFirst($course->course_video); 
        // }
        return view('instructor.course.create_video', compact('course', 'videos'));
    }

    public function instructorCourseVideoDelete(Request $request)
    {   
        // $course = Course::find($course_id);
        // pr($request->all());
        $videos = CourseVideos::where('id',$request->id)->delete();
        // $video = null;
        // if($course->course_video)
        // {
        //     $video = $this->model->getvideoinfoFirst($course->course_video); 
    ...
    //         //delete old file
    //         if ($input['old_course_image'] && Storage::exists($input['old_course_image'])) {
    //             Storage::delete($input['old_course_image']);
    //         }

    //         if ($input['old_course_image'] && Storage::exists($input['old_thumb_image'])) {
    ...
    public function instructorCourseImageSave(Request $request)
    {
        ...
        */
        ...
        //  $check=Storage::disk('public_profile')->put($imageName, base64_decode($image));
        // }

        if($request->hasFile('course_image')){
            $image_name = time().'.'.$request->course_image->getClientOriginalExtension();
            // pr($image_name);
            $check=Storage::disk('public_course')->put($image_name, file_get_contents($request->course_image));
            $course = Course::find($course_id);
        ...
        // Storage::disk('public_profile')->put($imageName, base64_decode($image));
    ...
    public function instructorCourseInfoSave(Request $request)
    {
        ...
        $course->instructor_id = (int)$request->instructor;
    ...
    public function save_youtube_link(Request $request)
    {
         $video=new CourseVideos();
         $video->video_name=$request->course_video;
         $video->course_id=$request->course_id;
         $video->video_title=$request->video_title;
         $video->uploader_id=auth()->user()->id;
         $video->save();
         Course::where('id',$request->course_id)->update(['course_video'=>1]);
         return redirect()->back()->with('success','video link successfully added');
    }
