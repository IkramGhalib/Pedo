...
class GroupController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('search')){
            ...
            $categories = Group::where('name', 'LIKE', '%' . $search . '%')
                           ->paginate($paginate_count);
        }
        else {
            $categories = Group::paginate($paginate_count);
        }
    ...
    {
        $master_category = MasterCategory::get();
        $master_course = MasterCourse::get();
        $instructor = Instructor::get();
        if($category_id) {
            $category = Group::find($category_id);
        }else{
            $category = $this->getColumnTable('groups');
        }
        return view('admin.groups.form', compact('category','master_category','master_course','instructor'));
    }
